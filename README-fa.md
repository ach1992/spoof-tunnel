...
>
> این فایل نسخهٔ نهایی فارسی README برای **Ubuntu / Debian** است و بر پایهٔ **کد واقعی برنامه** تنظیم شده است، نه صرفاً مثال‌های README انگلیسی. بنابراین فرمان‌های این فایل با CLI واقعی باینری هماهنگ هستند.
...
Spoof Tunnel یک تونل لایه ۳ / لایه ۴ با مدل client/server است که از raw packet، رمزنگاری و mutual spoofing استفاده می‌کند. برنامه در یک باینری واحد اجرا می‌شود و حالت client یا server از داخل فایل کانفیگ مشخص می‌شود.
...
3. باینری یک فایل واحد است و subcommand جدا برای `server` و `client` ندارد.
4. طبق کد واقعی برنامه، فلگ درست اجرای کانفیگ **`-config`** است.
5. طبق کد واقعی برنامه، ساخت کلید با فلگ **`-generate-keys`** انجام می‌شود.
...
3. public keyها را بین دو سمت جابه‌جا کنید.
...
فرمان‌های اصلی این پروژه بر اساس کد واقعی برنامه این‌ها هستند:
